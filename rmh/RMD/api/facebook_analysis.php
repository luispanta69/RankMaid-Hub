...
// Now pulls data from `universal_ad_data` table.
function get_and_process_ads_from_db($pdo, $start_date, $end_date) {
    $sql = "SELECT
                ...
                frequency,
                conversions AS results,
                cpa AS cost_per_result,
                spend AS amount_spent_usd,
                ctr AS ctr_all
            FROM universal_ad_data";
    $params = [];
    $whereClauses = ["platform = 'facebook'"]; // Ensure we only get Facebook data

    if ($start_date && $end_date) {
        $whereClauses[] = "reporting_date BETWEEN :start_date AND :end_date";
    ...
    // Add 'result_type' as it's not directly available in universal_ad_data
    foreach ($all_ads as &$ad) {
        $ad['result_type'] = ''; // Placeholder
    }
    unset($ad); // Break the reference with the last element
        ...
        foreach ($ad_sets as $set) {
             ...
             // Lowered threshold from >100 to >50 to be safer for short ranges
                 ...
                 $opportunities[] = [
                     ...
                     'roas' => 0,
                     'spend' => $set['spend'],
                     'reason' => 'Budget Bleeder: Zero Results / High Spend',
                     'type' => 'bleeder'
                 ];
             }
        }
